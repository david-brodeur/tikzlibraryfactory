...
\ProvidesTikzLibrary{factory.pallet}[2025/05/05 v1.0 Factory agv robot library]
...
\newcommand{\drawAGV}[4]{%
  ...
  \begin{scope}[shift={(\item@x,\item@y)}, rotate=\item@orientation]
    ...
    \pgfmathsetmacro{\cornerRadius}{10}%
    \pgfmathsetmacro{\wheelOffset}{0.3}%
    \pgfmathsetmacro{\wheelRadius}{0.3}%
    \pgfmathsetmacro{\wheelThickness}{0.4}%

    \pgfmathsetmacro{\wheel@front@x}{\item@xend - \wheelOffset - \wheelRadius}%
    \pgfmathsetmacro{\wheel@front@y}{\item@yend}%

    \pgfmathsetmacro{\wheel@rear@x}{\item@xorigin + \wheelOffset + \wheelRadius}%
    ...
    \foreach \i in {-1,1} {
      ...
      \begin{scope}[shift={(\wheel@front@x, \i*\wheel@front@y)}, rotate=\robot@wheelsOrientation]
        \draw[fill=black!60, draw=black] (-\wheelRadius, -\wheelThickness/2) rectangle (\wheelRadius,\wheelThickness/2);
      \end{scope}
    ...
    % Draw robot base
    \draw[rounded corners=\cornerRadius, fill=\item@color!60, draw=black, line width=0.3pt] (\item@xorigin, \item@yorigin) rectangle (\item@xend, (\item@yend);%
